<?php
	...
	$cdCustomerDetails = new CustomerDetails($adBillingAddress, $_SESSION['email'], $_SESSION['phone'], $_SERVER["REMOTE_ADDR"]);
	...
	else
	{
		switch ($goGatewayOutput->getStatusCode())
		{
			...
			case 3: 
				...
				$PaREQ = $_SESSION['pares'] = $tomTransactionOutputMessage->getThreeDSecureOutputData()->getPaREQ();
				$CrossReference = $_SESSION['crossReference'] = $tomTransactionOutputMessage->getCrossReference();
				?>
<form action="https://gw1.cardsaveonlinepayments.com:4430/ACS/Default.aspx" method="post" target=ifr>
<?
$post=array("MD"=>$CrossReference,"PaReq"=>$PaREQ,"TermUrl"=>$SiteSecureBaseURL."cardsave_direct/ThreeDSecureLandingPage.php");
...
<iframe id=ifr name=ifr height=500 width=500 frameborder=0></iframe>
<?
die;
				break;
