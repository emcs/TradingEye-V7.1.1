...
use vars qw (@ISA @EXPORT_OK $VERSION $DEBUG $PFPRO
             ...
             $USER $VENDOR $PARTNER $PWD $TRXTYPE $TENDER $TIMEOUT);
...
$PARTNER      = "";
$PWD          = "";
# NOTE: If you do not modify the above account values, you will
#	experience User Authentication failures when running the
#	test script via make test or perl test.pl!
...
sub timeIt {
  ...
  if ( !$data->{USER} || !$data->{VENDOR}
    || !$data->{PARTNER} || !$data->{PWD} )
  {
    print "\nYou will need to set the USER, VENDOR, PARTNER and PASSWORD\n";
    print "to your User, Vendor, Partner and Password as specified when you\n";
    print "signed up with the Payflow Pro service.\n";

    print "\nPlease set these variables in PFProAPI.pm (near the top) to your\n";
    print "values before continuing to avoid user authentication failures:\n";
    print "\t\$USER\n\t\$VENDOR\n\t\$PARTNER\n\t\$PWD\n";
    print "These variables are set near the top of PFProAPI.pm.\n\n";
  }
...
SIMPLE (non-shared environment):
  ...
  $data->{ACCT}    = "5105105105105100";  # Test Card
  $data->{EXPDATE} = "1209";
  $data->{AMT}     = "1.00";
  $data->{USER}    = "user";
  $data->{VENDOR}  = "vendor";
...
You must be a knowledgeable Perl programmer to effectively use this module.  

Before using this module, you should modify the default configuration 
in the head of this file (PFProAPI.pm) to reflect your own setup.
{!+
At the very least you will need to obtain a test account with Verisign 
...
module successfully.  If you are using a shared installation (i.e, an ISP),
...
appropriate numbers in their own scripts by calling call_pfpro directly.
      ...
      -!}
      pfpro maps a Perl call to the internal Payflow Pro Client library.
      The parameters are the same as the binary and library client.  The
      return value is a list of name=value pairs, exactly as you would
      ...
      Note that the payment system, while it does use name=value pairs,
      ...
      The getparams() convenenience function does the inverse: converts an
      encoded string to an associative array for easier processing.
