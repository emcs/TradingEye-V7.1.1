...
and corresponding examples.  The Payflow Pro JNI Client enables processing
of realtime credit card transactions.

{!+
The Payflow Pro JNI Client provides access to Verisign Payment Services.
-!}
...
{!+
Please see http://www.verisign.com/payment for full instructions 
...
Please see the enclosed general client documentation for more information.
  ...
  and that the JNI Library may be found in the path:
        ...
        UNIX:
           ...
           Library Path which enables libpfprojni.so and libpfpro.so to
           be found when executing the Java Native Interface for Payflow Pro Client.

           Note: If you copy the libpfprojni.so and libpfpro.so files
                 to a directory in the /usr/lib path, then you do not need
                 to set the Runtime Library Path as described below. 
                 However, you must set the CLASSPATH in any case.
           ...
           shared objects libpfprojni.so and libpfpro.so have been installed.
...
(you may need to change the user, vendor, partner, and password to what you signed up with):
        java PFProJava test-payflow.verisign.com 443 "USER=user&VENDOR=vendor&PARTNER=partner&PWD=password&TRXTYPE=S&TENDER=C&ACCT=5105105105105100&EXPDATE=1209&AMT=1.23&ZIP=12345&comment1=Test JNI Transactions" 30
...
the steps listed above to compile and run the test transaction using the PFProJava example.
...
-------
        ...
        to use the calls. Be sure that your classpath
