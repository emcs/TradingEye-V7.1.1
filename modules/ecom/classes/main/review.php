...
#CONSTRUCTOR
	...
	function m_reviewCheckout()
	{
		 ...
		 if($rowCartCount>0)
		{
			...
			for($i=0;$i<$rowCartCount;$i++)
			{
				...
				if($vDiscoutPerItem>0 && $comFunc->m_checkCustomerType()==1 && ENABLE_WHOLESALE==1)
				{
	...
	function hsbcmessages()
	{
		if (isset($_POST["CpiResultsCode"])) {
		switch ($_POST["CpiResultsCode"]) {
		case 0:
		$trans = 'The transaction was approved.';
		$retUrl=SITE_SAFEURL."ecom/index.php?action=checkout.process&mode=".$_POST['OrderId']."&phpsessid=".session_id();;
		$this->libFunc->m_mosRedirect($retUrl);
		break;
		case 1:
		$trans = 'The transaction was cancelled by the user.';
		break;
		case 2:
		$trans = 'The processor declined the transaction for an unknown reason.';
		break;
		case 3:
		$trans = 'The transaction was declined because of a problem with the card. For example,an invalid card number or expiration date was specified.';
		break;
		case 4:
		$trans = 'The processor did not return a response.';
		break;
		case 5:
		$trans = 'The amount specified in the transaction was either too high or too low for the processor.';
		break;
		case 6:
		$trans = 'The specified currency is not supported by either the processor or the card.';
		break;
		case 7:
		$trans = 'The order is invalid because the order ID is a duplicate.';
		break;
		case 8:
		$trans = 'The transaction was rejected by FraudShield.';
		break;
		case 9:
		$trans = 'The transaction was placed in Review state by FraudShield';
		break;
		case 10:
		$trans = 'The transaction failed because of invalid input data.';
		break;
		case 11:
		$trans = 'The transaction failed because the CPI was configured incorrectly.';
		break;
		case 12:
		$trans = 'The transaction failed because the Storefront was configured incorrectly.';
		break;
		case 13:
		$trans = 'The connection timed out.';
		break;
		case 14:
		$trans = 'The transaction failed because the cardholders browser refused a cookie.';
		break;
		case 15:
		$trans = 'The customers browser does not support 128-bit encryption.';
		break;
		case 16:
		$trans = 'The CPI cannot communicate with the Secure ePayment engine.';
		break;
		}
		} else { $trans = ""; }
		return $trans;
