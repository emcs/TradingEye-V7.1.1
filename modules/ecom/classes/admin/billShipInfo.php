...
#CONSTRUCTOR
	...
	{
		$this->obDb->query= "select vFirstName,iCustmerid_PK  FROM ".CUSTOMERS." WHERE vEmail = '".$this->request['email']."' AND vPassword=PASSWORD('".$this->request['password']."')";
	...
	{
		$this->obDb->query= "SELECT iCustmerid_PK  FROM ".CUSTOMERS." WHERE vEmail = '".$this->request['email']."'";
		$qryRs = $this->obDb->fetchQuery();
		$rCount=$this->obDb->record_count;
		if($rCount>0) 
		{
	...
	function m_billShipInfoForm()
	{
		...
		$this->ObTpl->set_block("TPL_USER_FILE","TPL_HSBC_BLK","hsbc_blk");
		$this->ObTpl->set_block("TPL_USER_FILE","TPL_BARCLAY_BLK","barclay_blk");
		...
		$this->ObTpl->set_var("LANG_VAR_YES",LANG_YES);
...
$this->ObTpl->set_var("TPL_VAR_NEXTFORMMODE",$_SESSION['NextFormMode']);
	...
	function m_saveBillShipInfo() {
		...
		if(isset($this->request['member_points']) && $this->request['member_points']=='1') 
		{
		...
		else
		{
			...
			$_SESSION['usedMemberPoints']="";
			$_SESSION['memberPointsUsedAmount']="";
		}
