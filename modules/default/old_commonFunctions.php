...
#FUNCTION TO CALCULATE INTERNATIONAL POSTAGE COST
  ...
  #FUNCTION WILL RETURN OPTIONS ATTACHED WITH PRODUCTID
	function m_getOptions($kit,$selectedarray="",$selectedOrderidArray="",$adminSection=0)
	{
		...
		$this->ObTpl->set_file("TPL_VAR_OPTFILE",MODULES_PATH."ecom/templates/main/options.tpl.htm");
	...
	{
		$this->ObTpl=new template();
		$this->ObTpl->set_file("TPL_VAR_CHOICEFILE",MODULES_PATH."ecom/templates/main/choices.tpl.htm");
	...
	function m_getVolDiscount()
	{
		...
		$this->ObTpl=new template();
		$this->ObTpl->set_file("TPL_VAR_DISCOUNTFILE",MODULES_PATH."ecom/templates/main/volDiscount.tpl.htm");
	...
	{
		$strChoices="";
		$this->obDb->query ="SELECT iChoiceid_PK,vDescription,vChoiceVal,fPrice,iQty  FROM ".CHOICES.", ".TEMPCHOICES." WHERE iTmpChoiceId_FK=iChoiceid_PK AND iTmpCartId_FK='".$this->cartId."' ";
	...
	function m_postagePrice()
	{
		...
		elseif(DEFAULT_POSTAGE_METHOD=='range')
		{
			...
			for($i=0;$i<$rsCount;$i++)
			{
				...
				if($rsPostage[$i]->vField1<=$this->grandTotal && $rsPostage[$i]->vField2=='0')
				{
...
//		$curTime=time();
	...
	function m_sendDetails($email,$admin=0)
	{
		$libFunc=new c_libFunctions();
	...
	//	$uniqID=uniqid (3);
		...
		if($rCount>0) 
		{
			...
			$message.="<br />Password:&nbsp;".$qryResult[0]->vPassword;
	...
	function m_sendDetails_trade($email,$admin=0)
	{
		$libFunc=new c_libFunctions();
		$this->obDb->query= "SELECT vFirstName,vLastName,vEmail,vPassword  FROM ".CUSTOMERS." WHERE vEmail = '".$email."'";
		$qryResult = $this->obDb->fetchQuery();
	...
	function m_orderProductOptions()
	{
		...
		if($optCount>0)
		{
			for($i=0;$i<$optCount;$i++)
			{
				...
				$strOptions.=$rsOptions[$i]->vName.": ".$rsOptions[$i]->vItem." Product sku (".$rsOptions[$i]->vOptSku.")";
	...
	function m_dspError()
	{
		...
		$this->ObTpl=new template();
		$this->ObTpl->set_file("TPL_ERROR_TFILE",MODULES_PATH."default/templates/main/error.tpl.htm");
