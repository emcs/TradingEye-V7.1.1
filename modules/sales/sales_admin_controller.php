...
//--------------------		
			...
			break;
			case "email":
			$obEmailInterface=new c_emailInterface();
			$obEmailInterface->obTpl=$this->obTpl;
			$obEmailInterface->obDb=$this->obDb;
			$obEmailInterface->request=$this->request;

			$obEmailDb=new c_emailDb();
			$obEmailDb->obDb=$this->obDb;
			$obEmailDb->request=$this->request;
			switch($action[1])
			{
				case "home":
				$obEmailInterface->emailTemplate=$this->templatePath."emailCampHome.tpl.htm";
				$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_dspEmails());
				break;
				case "leadHome":
				$obEmailInterface->leadTemplate=$this->templatePath."leadHome.tpl.htm";
				$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_dspLeads());
				break;
				case "dspForm":
				$obEmailInterface->emailTemplate=$this->templatePath."emailCampEditor.tpl.htm";
				$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_emailBuilder());
				break;
				case "leadForm":
				$obEmailInterface->leadTemplate=$this->templatePath."createLead.tpl.htm";
				$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_leadBuilder());
				break;
				case "setsession":
					
				if (isset($this->request['object'])){	
					if($this->request['object']=='txtName'){
					$_SESSION['listname']= $this->request['value'];
					}elseif($this->request['object']=='leadBuyers'){
						if($this->request['value']=="yes")
						{
							$_SESSION['yeschecked']= 1;
							unset($_SESSION['nochecked']);	
						}else{
							$_SESSION['nochecked']= 1;
							unset($_SESSION['yeschecked']);
						}
					}elseif($this->request['object']=='start_date'){
						$_SESSION['fromdate']= $this->request['value'];
					}elseif($this->request['object']=='end_date'){
						$_SESSION['todate']= $this->request['value'];
					}
				...
				break;
				case "leadForm1":
					$obEmailInterface->leadTemplate=$this->templatePath."createLead2.tpl.htm";
					$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_leadBuilder1());
				break;
                case "dspCustomerlist":
					$obEmailInterface->leadTemplate=$this->templatePath."customerlist.tpl.htm";
					$obEmailInterface->m_dspCustomerlist();
				break;
				case "update":
					$this->libfunc->check_token();
					if(isset($this->request['mode']) && $this->request['mode']=="edit")
					{
						if(!$obEmailInterface->m_verifyEdit())
						{
							$obEmailDb->m_updateEmail();
						}
						else
						{
							$obEmailInterface->request['id']=$this->request['id'];
							$obEmailInterface->emailTemplate=$this->templatePath."emailCampEditor.tpl.htm";
							$this->obTpl->set_var("TPL_VAR_BODY",$obEmailInterface->m_emailBuilder());
						}
					}
					...
					{
						if(!$obEmailInterface->m_verifyInsert())
						{
							$obEmailDb->m_insertEmail();
						}
						else
						{
							$obEmailInterface->emailTemplate=$this->templatePath."emailCampEditor.tpl.htm";
				...
				case "updateLead1":
					$this->libfunc->check_token();
					$obEmailDb->m_insertLead1();
				break;
				case "deleteLead":
					$this->libfunc->check_token();
					$obEmailDb->m_emailDeleteLead();
				break;
				case "deleteLeadMain":
					$this->libfunc->check_token();
					$obEmailDb->m_emailDeleteLeadMain();
				break;
				

				case "sendmail":
					$this->libfunc->check_token();
					$obEmailDb->m_sendMail();
				break;
			}#END SWITH 1
			break;#END EMAIL
